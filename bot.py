...
def project_gpt_enabled(topic_name: str) -> bool:
    """
    Проверяет, включён ли GPT-анализ для проекта.
    По умолчанию True — выключить можно через /gpt off в топике.
    Требует поле 'GPT активен' (Checkbox) в базе Проекты в Notion.
    """
    try:
        res = notion.databases.query(
            database_id=PROJECTS_DB,
        ...
        pages = res.get("results", [])
        if not pages:
            return True  # проект не найден → по умолчанию включён
        prop = pages[0]["properties"].get("GPT активен")
        if prop is None:
            return True  # поле ещё не добавлено → включён
        return prop.get("checkbox", True)
    except Exception as e:
        logger.error(f"project_gpt_enabled: {e}")
        return True


def set_project_gpt(topic_name: str, enabled: bool) -> bool:
    """Включает/выключает GPT-анализ для проекта в Notion."""
    try:
        res = notion.databases.query(
            database_id=PROJECTS_DB,
            filter={"property": "Проект", "title": {"equals": topic_name}},
        ...
        notion.pages.update(
            page_id=pages[0]["id"],
            properties={"GPT активен": {"checkbox": enabled}},
        )
        return True
    except Exception as e:
        logger.error(f"set_project_gpt: {e}")
        return False
...
- status (string): один из статусов ниже
...
СТАТУСЫ — назначай ТОЛЬКО при явном указании, иначе ставь "🆕 Новая":
- "🆕 Новая" — по умолчанию, если статус не ясен
- "✂️ Монтаж" — ТОЛЬКО если явно сказано "в монтаже", "монтирую", "начинаю монтаж"
- "🔄 Правки" — ТОЛЬКО если явно просят внести правки: "правки", "переделай", "исправь"
- "👁️ Финальный просмотр" — ТОЛЬКО если явно "на финале", "финальный просмотр"
- "📤 Отправлено клиенту" — ТОЛЬКО если явно "отправил клиенту", "выслал"
- "🟡 На согласовании" — ТОЛЬКО если явно "на согласовании", "согласуем"
- "🎨 Ждём цвет" — ТОЛЬКО если явно "ждём цвет", "отправили на колористику", "на цвете"
- "🔊 Ждём звук" — ТОЛЬКО если явно "ждём звук", "отправили на звук", "у звукача"
- "✨ Ждём графику" — ТОЛЬКО если явно "ждём графику", "у моушн-дизайнера"
- "✅ Сдано" — ТОЛЬКО если явно "сдали", "готово", "сдано", "отправили финал"
- "⏸️ Заморожено" — ТОЛЬКО если явно "заморожено", "стоп", "на паузе"

СТРОГИЕ ПРАВИЛА:
1. Упоминание слова "звук", "цвет", "графика" в обычном разговоре (обсуждение качества, идей) НЕ означает статус ожидания
2. Обсуждение идей, концепций, пожеланий — это НЕ задача (is_task=false)
3. Вопросы, уточнения, реакции на чужие сообщения — НЕ задача
4. Если сомневаешься — is_task=false, confidence<0.5
5. confidence >= 0.7 только если сообщение содержит явное действие или поручение
"""
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # 1. Игнор-префикс: // или -- в начале → не анализируем
    stripped = text.strip()
    if stripped.startswith("//") or stripped.startswith("--"):
        return

    # 2. Слишком короткое сообщение (реакции, "ок", "👍") → не анализируем
    if len(stripped) < 15:
        return

    # 3. GPT выключен для этого топика → тихо пропускаем
    if not project_gpt_enabled(topic):
        return
...
# ─────────────────────────────────────────────────────────────────────────────
# Команда /report — компактный статус-отчёт
# ─────────────────────────────────────────────────────────────────────────────
def _fmt_compact_line(tasks: list[dict]) -> str:
    """Один компактный ряд: ПРОЕКТ #выпуск, ПРОЕКТ2 #выпуск2, ..."""
    parts = []
    for t in tasks:
        p       = t["properties"]
        project = _get_select(p, "Проект") or "?"
        episode = _get_select(p, "Выпуск")
        ep_str  = f" \\#{escape_md(episode)}" if episode and episode != "—" else ""
        parts.append(f"{escape_md(project)}{ep_str}")
    return ", ".join(parts)


async def cmd_report(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """
    /report — компактная сводка по статусам (одна строка на статус).
    Для подробного GPT-отчёта используй /digest.
    """
    msg = update.message
    tid = msg.message_thread_id
    ...
    active_sections = [
        ("✂️ Монтаж",    "✂️ Монтаж"),
        ("🔄 Правки",    "🔄 Правки"),
        ("🟡 Согласов.", "🟡 На согласовании"),
        ("👁️ Финал",    "👁️ Финальный просмотр"),
        ("📤 Клиенту",  "📤 Отправлено клиенту"),
        ("🎨 Ждём цвет","🎨 Ждём цвет"),
        ("🔊 Ждём звук","🔊 Ждём звук"),
        ("✨ Графика",   "✨ Ждём графику"),
        ("🆕 Новые",    "🆕 Новая"),
    ]

    lines = [f"📊 *{datetime.now().strftime('%d.%m')}*"]
    total = 0
    for label, status in active_sections:
        tasks = get_tasks_by_status(status)
        if tasks:
            lines.append(f"{label} *({len(tasks)})* — {_fmt_compact_line(tasks)}")
            total += len(tasks)
    ...
    if overdue:
        ...
        for t in overdue:
            p  = t["properties"]
            nm = escape_md(_get_title(p)[:40] or "?")
            pr = escape_md(_get_select(p, "Проект"))
            dl = ((p.get("Дедлайн") or {}).get("date") or {}).get("start", "?")
            lines.append(f"  • {pr} — {nm} | 📅 {dl}")

    lines.append(f"\n_Активных: {total}_ · /digest для подробного отчёта")
...
async def cmd_digest(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """
    /digest — GPT-отчёт за сегодня (или за вчера если сегодня нет активности).
    Генерирует аналитику по всем проектам и присылает в чат.
    """
    msg = update.message
    tid = msg.message_thread_id
    await msg.reply_text("⏳ Генерирую отчёт...", message_thread_id=tid)

    target = date.today()
    summary = await generate_daily_report(target)
    if not summary:
        # Нет событий сегодня — пробуем вчера
        target = date.today() - timedelta(days=1)
        summary = await generate_daily_report(target)

    if not summary:
        await msg.reply_text(
            "📭 За последние 2 дня активности не найдено.",
            message_thread_id=tid,
        )
        return

    header = f"📋 *Отчёт за {target.strftime('%d.%m.%Y')}*\n\n"
    full_text = header + summary
    chunks = [full_text[i:i+4000] for i in range(0, len(full_text), 4000)]
    for chunk in chunks:
        await msg.reply_text(
...
async def cmd_smart_project(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    RESERVED = {
        ...
        "deadline", "assign", "waiting", "report", "digest", "help",
        "start", "topicid", "task", "s", "test", "setup", "gpt",
...
async def cmd_gpt(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """
    /gpt on  — включить GPT-анализ сообщений в этом топике (по умолчанию включён)
    /gpt off — выключить GPT-анализ (бот перестаёт создавать задачи из сообщений)
    """
    msg = update.message
    tid = msg.message_thread_id
    ...
    if not context.args or context.args[0].lower() not in ("on", "off"):
        await msg.reply_text(
            "🤖 Управление GPT-анализом:\n"
            "`/gpt off` — бот перестаёт создавать задачи из сообщений\n"
            "`/gpt on`  — включить обратно\n\n"
            "_Команды, кнопки и /report работают в любом режиме._",
            parse_mode="Markdown",
            message_thread_id=tid,
        )
        return
    ...
    topic = None
    if tid:
        topic = get_project_name(tid)
    if not topic:
        await msg.reply_text(
            "❓ Топик не зарегистрирован.\nСначала: `/setup Название`",
            parse_mode="Markdown",
            message_thread_id=tid,
        )
        return
    ...
    ok = set_project_gpt(topic, enabled)
    if ok:
        emoji  = "✅" if enabled else "🔕"
        status = "включён" if enabled else "выключен"
        note   = "" if enabled else "\n_Бот всё ещё реагирует на команды (/t, /s, /d…)_"
        await msg.reply_text(
            f"{emoji} GPT-анализ *{status}* для *{escape_md(topic)}*{note}",
            parse_mode="Markdown",
            message_thread_id=tid,
        )
    else:
        await msg.reply_text(
            "❌ Не удалось обновить.\n\n"
            "Добавь поле *GPT активен* (тип: Checkbox) в базу *Проекты* в Notion — "
            "и повтори команду.",
            parse_mode="Markdown",
            message_thread_id=tid,
        )
...
def main() -> None:
    ...
    # Расписание — только алерты по дедлайнам и ожиданиям
    # Дайджесты отключены: используй /report и /digest по запросу
